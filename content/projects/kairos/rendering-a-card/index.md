...
As we saw in the [previous post](projects/kairos/making-a-3d-card-in-blender/), each card has three materials:
...
To adjust each of these materials to its maximum potential, I will create three custom shaders using the pipeline that the game will use: '[Universal RP](https://docs.unity3d.com/Packages/com.unity.render-pipelines.universal@12.1/manual/index.html)', specifically version 12.1.

The simplest shader of the three is the one in charge of drawing the border of the card, which only has to draw one color:
...
Shader "Card/Border"
{
  ...
  Properties
  {
    // [MainColor] allows Material.color to use the correct properties.
  ...
  SubShader
  {
    ...
    // different data about the material. Each pass has its own
...
The shader for the front of the card, in its first version, is similar to the previous one but with an additional layer. It consists of three layers which, from back to front, are:
...
These effects depend on the type of UV mapping your geometry uses. In my case, as we saw in the [previous post](#rendering_a_card), I use a cubic projection; therefore, if I modify one axis of the coordinates, I know the texture will be displaced along that axis.
...
Finally, we need to add something fundamental: text. For this, we will use [TextMeshPro](https://docs.unity3d.com/Manual/com.unity.textmeshpro.html), as it provides much better quality and more options than Unity's default text. To install it, simply add it using the 'Package Manager'.
...
* Different [blending functions](https://photoshoptrainingchannel.com/blending-modes-explained/) for the effect layers. While I have used a simple lerp here, many other blending modes can yield interesting results.
