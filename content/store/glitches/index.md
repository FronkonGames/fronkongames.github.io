...
* [Bad Signal](#bad-signal), glitch your screen like it's 1999!
* [JPEG](#jpeg), blocky compression and color bleeding like lossy JPEG encoding.
* [Color Blindness](#colorblindness), simulates the most common vision defects.

{{< badge title="You can obtain each effect separately, but if you want more effects, you might be interested in **'[GLITCHES BUNDLE](https://assetstore.unity.com/packages/vfx/shaders/fullscreen-camera-effects/glitches-bundle-295977?aid=1101l9zFC&utm_source=aff)'** where you can find them all at a special price!" color="secondary">}}
{.h3}
...
{{< asset-header store="https://assetstore.unity.com/packages/vfx/shaders/fullscreen-camera-effects/glitches-bad-signal-384514" demo="https://fronkongames.github.io/demos-glitches/bad-signal/" youtube="GKogrJjXZ4g" warn="assets used in video and demo are not included">}}

Simulates a corrupted digital video signal with **chromatic aberration**, **broken scanlines** and **block noise**. The effect combines two independent glitch stages, **lines** and **blocks**, each with its own speed, chromatic aberration, glow, blend mode and tint, producing the characteristic visual degradation of a failing video transmission.

The **lines** stage generates hash-driven scanlines that can be rotated, shifted and tinted. A per-stage glow system adds configurable bloom around active scanlines with independent width and sampling pass count.

The **blocks** stage quantizes the screen into a grid of cells, each displaced and color-shifted by a 3D hash function, creating the blocky corruption patterns typical of damaged video memory or poor digital reception.

{{< alert type="info" >}}
Each stage operates independently: you can disable either by setting its intensity to 0, or combine both for maximum signal degradation. The per-stage glow uses a multi-pass sampling approach where pass count controls smoothness and glow width controls spread.
{{< /alert >}}
...
1. Locate your **Universal Renderer Data** asset.
2. Click **Add Renderer Feature** and select **Fronkon Games > Glitches > Bad Signal**.
...
1. Create a **Volume** component (Global or Local).
2. In the Volume component, create or assign a **Volume Profile**.
3. Click **Add Override** and select **Fronkon Games > Glitches > Bad Signal**.
4. Enable the '**Intensity**' parameter (and any others you wish to modify).

#### Dual-Stage Signal Corruption

The effect implements two independent glitch stages that composite sequentially, each with full per-stage control over speed, chromatic aberration, glow, blending and tinting.

{{< table >}}
| **Stage** | **Technique** | **Characteristic** | **Typical Use Case** |
|---|---|---|---|
| Lines | Hash-driven scanlines with rotation, shift and glow | Horizontal/rotated tearing with chromatic fringing | Scanline interference, sync loss, CRT noise |
| Blocks | 3D hash grid quantization with displacement and glow | Blocky rectangular corruption with color shifting | Data loss, compression damage, memory corruption |
{{< /table >}}

Both stages share the same hash-based noise foundation but operate at different scales: Lines work in scanline space (1D hash per row), while Blocks work in a 2D grid (3D hash per cell). This produces distinct artifact patterns that combine organically.

#### Parameter Configuration

{{< image src="badsignal_0.png" wrapper="col-10 mx-auto">}}
...
| **Parameter** | **Range** | **Default** | **Effect** |
|---|---|---|---|
| **Lines Intensity** | 0.0 to 1.0 | 1.0 | Intensity of the scanline corruption |
| **Lines Speed** | 0.1 to 50.0 | 10.0 | Temporal evolution rate of the scanline pattern |
| **Lines Chromatic** | 0.0 to 1.0 | 1.0 | Chromatic aberration strength applied to scanlines |
| **Lines Glow** | 0.0 to 1.0 | 0.2 | Bloom intensity around active scanlines |
| **Lines Glow Width** | 0.0 to 1.0 | 0.5 | Spread of the glow around scanlines |
| **Lines Glow Passes** | 1 to 16 | 4 | Sampling passes for glow smoothness. Higher values produce smoother glow at increased cost |
| **Lines Count** | 1.0 to 512.0 | 100.0 | Number of scanlines across the screen |
| **Lines Shift** | 0.0 to 1.0 | 0.1 | Horizontal displacement strength of scanlines |
| **Lines Angle** | 0.0 to 360.0 | 45.0 | Rotation angle of the scanline pattern in degrees |
| **Lines Threshold** | 0.0 to 1.0 | 0.04 | Mix threshold that controls which scanlines become active |
| **Lines Blend** | Enum (18 modes) | Solid | Color blend operation for mixing scanlines with the original image |
| **Lines Tint** | Color (with alpha) | White (0.75 alpha) | Color tint applied to scanlines. Alpha controls the blend weight |
{{< /table >}}

{{< alert type="info" >}}
Lines Angle is specified in degrees in the inspector [0-360] and converted to radians internally. At 0° the scanlines are horizontal; at 90° they are vertical.
{{< /alert >}}
...
| **Parameter** | **Range** | **Default** | **Effect** |
|---|---|---|---|
| **Blocks Intensity** | 0.0 to 1.0 | 1.0 | Intensity of the block corruption |
| **Blocks Speed** | 0.1 to 50.0 | 10.0 | Temporal evolution rate of the block pattern |
| **Blocks Chromatic** | 0.0 to 1.0 | 1.0 | Chromatic aberration strength applied to blocks |
| **Blocks Glow** | 0.0 to 1.0 | 0.2 | Bloom intensity around active blocks |
| **Blocks Glow Width** | 0.0 to 1.0 | 0.5 | Spread of the glow around blocks |
| **Blocks Glow Passes** | 1 to 16 | 4 | Sampling passes for glow smoothness |
| **Blocks Count** | 0.1 to 32.0 | 10.0 | Number of grid cells per axis |
| **Blocks Shift** | 0.0 to 1.0 | 0.3 | Displacement strength of each block |
| **Blocks Threshold** | 0.0 to 2.0 | 0.5 | Mix threshold that controls which blocks become active |
| **Blocks Blend** | Enum (18 modes) | Solid | Color blend operation for mixing blocks with the image |
| **Blocks Tint** | Color (with alpha) | White (0.75 alpha) | Color tint applied to blocks. Alpha controls the blend weight |
{{< /table >}}
...
| **Parameter** | **Range** | **Default** | **Effect** |
|---|---|---|---|
| **Background Blend** | Enum (18 modes) | Multiply | Color blend operation for the background tint |
| **Background Tint** | Color | White | Color tint applied to the background before stage compositing |
{{< /table >}}
...
The effect integrates with Unity's Volume system for seamless runtime parameter modification. Access the **BadSignalVolume** component through the Volume Profile.

```csharp
using UnityEngine;
using UnityEngine.Rendering;
using FronkonGames.Glitches.BadSignal;
...
if (volumeProfile.TryGet(out BadSignalVolume volume))
{
    // Enable/disable effect
    volume.intensity.value = 1.0f; // 0.0 = disabled

    // Configure lines stage
    volume.linesIntensity.value = 1.0f;
    volume.linesSpeed.value = 10.0f;
    volume.linesChromatic.value = 1.0f;
    volume.linesCount.value = 100.0f;
    volume.linesShift.value = 0.1f;
    volume.linesAngle.value = 45.0f;
    volume.linesThreshold.value = 0.04f;
    volume.linesBlend.value = ColorBlends.Solid;
    volume.linesTint.value = new Color(1.0f, 1.0f, 1.0f, 0.75f);
    volume.linesGlow.value = 0.2f;
    volume.linesGlowWidth.value = 0.5f;
    volume.linesGlowPasses.value = 4;

    // Configure blocks stage
    volume.blocksIntensity.value = 1.0f;
    volume.blocksSpeed.value = 10.0f;
    volume.blocksChromatic.value = 1.0f;
    volume.blocksCount.value = 10.0f;
    volume.blocksShift.value = 0.3f;
    volume.blocksThreshold.value = 0.5f;
    volume.blocksBlend.value = ColorBlends.Solid;
    volume.blocksTint.value = new Color(1.0f, 1.0f, 1.0f, 0.75f);
    volume.blocksGlow.value = 0.2f;
    volume.blocksGlowWidth.value = 0.5f;
    volume.blocksGlowPasses.value = 4;

    // Configure background
    volume.backgroundBlend.value = ColorBlends.Multiply;
    volume.backgroundTint.value = Color.white;
...
The effect executes in a single render pass with **O(1)** per-pixel complexity. Both stages use analytical hash functions without texture lookups. When glow is enabled, the cost scales linearly with the glow pass count.

Performance considerations:

* Pass Count: 1 blit pass.
* Texture Samples: 2 per pixel (original + displaced sample per stage) + N glow samples per active pixel (N = glow passes).
* Branching: Threshold comparisons and glow keyword branches use UNITY_BRANCH for coherent warp execution.
* Memory: No additional textures or compute buffers.
* Glow Cost: When glow is disabled (Glow = 0), the glow code path is eliminated via shader keyword. When enabled, cost scales with Glow Passes.

{{< table >}}
| **Glow Passes** | **Estimated GPU Cost** | **Recommended Use Case** |
|---|---|---|
| 1-2 | < 0.3ms | Mobile, many simultaneous effects |
| 3-6 | 0.3-0.8ms | Desktop, balanced scenes |
| 8-16 | 0.8-2.0ms | Cinematic, dedicated post-processing |
{{< /table >}}
...
Pure scanline interference with no block corruption. Suitable for signal interference, sync loss, or CRT-style noise.

```
Intensity: 1.0
Lines: Intensity 1.0, Threshold 1.0, Count 512, Angle 175, Blend Solid, Tint White
Lines Speed 9.7, Chromatic 1.0, Shift 0.029, Glow 0
Blocks: Intensity 0 (disabled)
```

##### Interference Blocks

Pure block corruption with glow. Suitable for severe data loss or memory corruption scenarios.

```
Intensity: 1.0
Lines: Intensity 0 (disabled)
Blocks: Intensity 1.0, Threshold 1.961, Count 8, Blend Screen, Tint White
Blocks Speed 3.3, Chromatic 1.0, Shift 0.072, Glow 1.0, Glow Width 0.146, Glow Passes 2
Background Tint: Dark gray (0.137, 0.137, 0.137)
```

##### Cyber Glitch

Neon green cyberpunk aesthetic with high-speed scanlines and block corruption.

```
Intensity: 1.0
Lines: Intensity 1.0, Speed 20, Chromatic 1.0, Count 128, Shift 0.2, Angle 90
Lines Threshold 0.03, Blend Additive, Tint (0, 1, 0.5, 0.7), Glow 0.6, Width 0.8, Passes 8
Blocks: Intensity 0.8, Speed 15, Chromatic 0.8, Count 6, Shift 0.4, Threshold 0.8
Blocks Blend Additive, Tint (0, 1, 0.5, 0.5), Glow 0.3, Width 0.6, Passes 4
```

##### Signal Lost

Fading signal with soft glow scanlines and minimal block corruption. Suitable for atmospheric horror or transition sequences.

```
Intensity: 0.8
Lines: Intensity 0.7, Speed 2.0, Chromatic 0.6, Count 48, Shift 0.08, Angle 15
Lines Threshold 0.01, Blend Screen, Tint (1, 1, 1, 0.3), Glow 0.8, Width 1.0, Passes 12
Blocks: Intensity 0.2, Speed 1.0, Chromatic 0.2, Count 2, Shift 0.1, Threshold 1.5
Blocks Blend Screen, Tint (1, 1, 1, 0.2), Glow 0
```

##### Overclock

Fast teal/cyan digital chaos with high-speed scanlines and aggressive block displacement.

```
Intensity: 1.0
Lines: Intensity 0.9, Speed 40, Chromatic 0.9, Count 160, Shift 0.25, Angle 45
Lines Threshold 0.03, Blend Screen, Tint (0.4, 1, 0.8, 0.6), Glow 0.5, Width 0.7, Passes 8
Blocks: Intensity 0.8, Speed 30, Chromatic 0.7, Count 6, Shift 0.35, Threshold 0.6
Blocks Blend Screen, Tint (0.2, 0.8, 1, 0.5), Glow 0.3, Width 0.5, Passes 6
...
## 🗜️ JPEG {#jpeg}
{{< asset-header youtube="yhz7c2t8ipU" store="https://assetstore.unity.com/packages/vfx/shaders/fullscreen-camera-effects/glitches-jpeg-374942" demo="https://fronkongames.github.io/demos-glitches/jpeg/" warn="assets used in video and demo are not included">}}

Simulates **lossy JPEG-style** blocking and color degradation by running an **8×8 discrete cosine transform (DCT)** in **YCbCr** color space, quantizing frequency coefficients, and reconstructing the image per macroblock. The result is the familiar macro-blocking, ringing, and chroma smearing of heavy compression, without encoding an actual JPEG file.

The implementation follows a fixed **8×8 orthogonal DCT basis**, fills each block by sampling the source image over the current **macroblock** region, applies **horizontal then vertical** transforms, quantizes with a frequency-dependent step driven by **quality**, then inverts the DCT and picks the coefficient cell that corresponds to the current pixel within the block.

{{< alert type="info" >}}
This effect is **GPU-intensive**: every pixel reconstructs a full 8×8 block with many texture samples. Prefer lower resolutions or use sparingly on mobile hardware.
{{< /alert >}}
...
1. Locate your **Universal Renderer Data** asset.
2. Click **Add Renderer Feature** and select **Fronkon Games > Glitches > JPEG**.
...
1. Create a **Volume** component (Global or Local).
2. In the Volume component, create or assign a **Volume Profile**.
3. Click **Add Override** and select **Fronkon Games > Glitches > JPEG**.
4. Enable the '**Intensity**' parameter (and any others you wish to modify).

#### DCT and quantization pipeline

{{< table >}}
| **Stage** | **Operation** | **Controlled by** |
|---|---|---|
| Color space | RGB → YCbCr per sample | (fixed coefficients) |
| Block gather | 8×8 samples over each macroblock | Block size |
| Forward DCT | 2D DCT on Y, Cb, and Cr | — |
| Quantization | Round coefficients with frequency-dependent scale | Quality, luma/chroma multipliers |
| Inverse DCT | Reconstruct spatial block | — |
| Output | Pick coefficient slot for current pixel | Block size |
{{< /table >}}

**Compression** blends between the **original** image and the **reconstructed DCT** result before color grading. **Luma** and **chroma** quantization scales adjust how aggressively **Y** vs **Cb/Cr** coefficients are quantized.

#### Parameter Configuration

{{< image src="jpeg_0.jpg" wrapper="col-10 mx-auto">}}
...
| **Parameter** | **Range** | **Default** | **Effect** |
|---|---|---|---|
| **Quality** | 1 to 100 | 8 | Quantization scale. Higher values preserve more high-frequency detail (less blocky) |
| **Block size** | 2 to 64 (even) | 8 | Macroblock size in pixels. Larger blocks produce coarser structure |
| **Compression** | 0.0 to 1.0 | 1.0 | Blend between the original image and the DCT-compressed result |
| **Luma quantization** | 0.25 to 4.0 | 1.0 | Multiplier on luma (Y) quantization. Lower values increase luma blocking |
| **Chroma quantization** | 0.25 to 4.0 | 1.0 | Multiplier on chroma (Cb/Cr) quantization. Lower values increase chroma blocking |
{{< /table >}}

##### Color Grading

Standard color correction parameters apply after the JPEG pass.
...
The effect integrates with Unity's Volume system for seamless runtime parameter modification. Access the **JPEGVolume** component through the Volume Profile.

```csharp
using UnityEngine;
using UnityEngine.Rendering;
using FronkonGames.Glitches.JPEG;
...
if (volumeProfile.TryGet(out JPEGVolume volume))
{
    volume.intensity.value = 1.0f;

    volume.quality.value = 12;
    volume.blockSize.value = 16;
    volume.compression.value = 1.0f;
    volume.lumaQuantization.value = 1.0f;
    volume.chromaQuantization.value = 0.75f;
...
The effect executes in a **single** render pass, but each pixel performs a full **8×8 DCT** path with **many** texture samples and arithmetic (nested loops with a fixed 8×8 basis). Cost scales with resolution and is typically **much higher** than simpler fullscreen effects.

Performance considerations:

* Pass Count: 1 blit pass.
* Texture Samples: up to 64 per pixel for block gather (plus main sample for blending).
* ALU: heavy (DCT + quantization + inverse DCT per pixel).
* Memory: no additional textures or compute buffers.
...
Suitable for subtle web-cam or streaming degradation.

```
Intensity: 0.35, Quality: 24, Block size: 8
Compression: 1.0, Luma quantization: 1.0, Chroma quantization: 1.0
Brightness: 0.0, Contrast: 1.0, Saturation: 1.0
```

##### Heavy compression

Strong macro-blocking and color breakup for stylized flashbacks or datamoshing-style looks.

```
Intensity: 0.85, Quality: 4, Block size: 16
Compression: 1.0, Luma quantization: 0.5, Chroma quantization: 0.5
